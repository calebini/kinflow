...
Kinflow Durable Persistence Spec (Master Copy v0.2.8 — Convergence-Hardened)
...
- delivery_attempts.attempt_id:
  generated by daemon scheduler/outbound attempt context before adapter invocation (single authority);
  uniqueness source is daemon-generated ID namespace and DB PK enforcement.
- delivery_attempts.attempt_index:
  generated by daemon scheduler from reminder retry state and propagated unchanged through OutboundMessage.attempt_index and DeliveryResult attempt correlation.
- delivery_attempts.attempted_at_utc:
  write-time timestamp produced by persistence writer at INSERT using system UTC clock; MUST represent attempt row materialization time and MUST NOT be copied from adapter.result_at_utc.

Complete column mapping table:
- attempt_id <- daemon attempt context ID authority
- reminder_id <- resolved from adapter.delivery_id via scheduler attempt context (delivery_id->reminder_id binding)
- attempt_index <- scheduler computed outbound attempt_index
- attempted_at_utc <- persistence writer insert timestamp (system UTC)
- status <- adapter.status (DELIVERED->delivered; FAILED_TRANSIENT/FAILED_PERMANENT->failed; SUPPRESSED->suppressed; BLOCKED->blocked)
